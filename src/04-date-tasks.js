...
function timeSpanToString(startDate, endDate) {
  ...
  const hoursSEnd = endDate.getHours();
  const minEnd = endDate.getMinutes();
  const secEnd = endDate.getSeconds();
  const msEnd = endDate.getMilliseconds();
  return `${(hoursSEnd - hoursStart).toString().padStart(2, 0)}:${(
    minEnd - minStart
  )
    .toString()
    .padStart(2, 0)}:${(secEnd - secStart).toString().padStart(2, 0)}.${(
    msEnd - msStart
  )
    .toString()
    .padStart(3, 0)}`;
...
function angleBetweenClockHands(date) {
  ...
  const hourAngle = (hour * 360) / 12 + minutes * (30 / 60);
  const angle = (hourAngle - minuteAngle) % 360;
  return (Math.abs(Math.min(angle, 360 - angle)) / 180) * Math.PI;
}
